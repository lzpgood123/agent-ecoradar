...
def auto_level(p, ranking):
    score = p.get('total_score', total_score(p))
...
def main():
    ...
    for p in projects:
        ...
        elif p.get('review_state') in ('reviewed', 'auto-reviewed'):
            p['review_state'] = 'auto-indexed'
        elif p.get('review_state') == 'unreviewed':
            p['review_state'] = 'auto-indexed'
    ...
    candidates = sorted(candidates, key=lambda p: p.get('total_score', total_score(p)), reverse=True)
    ...
    for p in sorted(projects, key=lambda p: p.get('total_score', total_score(p))):
        ...
        if p.get('record_kind') == 'official-tool' or p.get('id') in curated_ids:
            continue
        weak = (
            p.get('total_score', total_score(p)) <= ranking['reject_max_score'] or
